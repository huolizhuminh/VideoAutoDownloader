...
public class UDPServer implements Runnable {
    ...
    public UDPServer(VpnService vpnService, ConcurrentLinkedQueue<ByteBuffer> outputQueue) {
        try {
    ...
    public void run() {
        try {
                    ...
                    while (keyIterator.hasNext()) {
                        ...
                        if (key.isValid()) {
                            try {
                                ...
                                VPNLog.e(TAG,"TcpProxyServer iterate SelectionKey catch an exception: %s"+ex);
                            }
                        }
                        keyIterator.remove();
                    }
                }catch (Exception e){
                   VPNLog.d(TAG,"failed to handle key "+e.getMessage());
                }
            ...
            VPNLog.d(TAG,"TcpProxyServer catch an exception: %s"+e.getMessage());
        } finally {
            this.stop();
            VPNLog.d(TAG,"TcpServer thread exited.");
        }
    }
