...
public class AndroidSegmentedControlView extends RadioGroup {
    ...
    //UI
    private int selectedColor = Color.parseColor("#F4840B");
    private int unselectedColor = Color.TRANSPARENT;
    private int unselectedTextColor = Color.parseColor("#F4840B");
    private int defaultSelection = -1;
    private boolean stretch = false;
    private int selectedTextColor = Color.WHITE;
    private boolean equalWidth = true;
    ...
    public AndroidSegmentedControlView(Context context, AttributeSet attrs) throws Exception {
        ...
        try {
            ...
            itemArray = attributes.getTextArray(R.styleable.MultipleSelectionButton_ascv_items);
            valueArray = attributes.getTextArray(R.styleable.MultipleSelectionButton_ascv_values);
    ...
    private void update() {
        ...
        for (RadioButton option : options) {
            if (equalWidth) {
                option.setWidth((int) (textWidth + (twoDP * 9)));
            }
            this.addView(option);
        }
    ...
    public void setSelectedItem(int item) {
        if (itemArray != null && itemArray.length > item)
            setByValue((String) itemArray[item]);
    }
